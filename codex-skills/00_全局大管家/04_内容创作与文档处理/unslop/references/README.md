...
git clone https://github.com/MohamedAbdallah-14/unslop.git
cd unslop
bash https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/hooks/install.sh            # macOS / Linux
pwsh https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/hooks/install.ps1           # Windows
```
...
For API-backed LLM mode, install the optional SDK with `pipx install 'unslop[llm]'`. See [`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/README.md`](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/README.md) for the full CLI surface.
...
Every fixture wins 3/3 runs. Reproduce with `python3 https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/evals/perceived_humanness.py --runs 3` (needs `ANTHROPIC_API_KEY`). Archived at `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/results/humanness/three-run-post-soul-fix-20260421.json`.
...
- **[https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/README.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/README.md)** — the Python package and standalone CLI.
...
- **[docs/research/](https://github.com/MohamedAbdallah-14/unslop/tree/v0.7.0/docs/research)** — public research index and August archive: 80 topic memos, 16 syntheses, four phase plans, plus the full [implementation trace](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/docs/research/IMPLEMENTATION_TRACE.md).
- **[CHANGELOG.md](./CHANGELOG.md)** — all releases.
- **[CONTRIBUTING.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/CONTRIBUTING.md)** — PR workflow, test gates, SSOT layout.
- **[https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/docs/RELEASING.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/docs/RELEASING.md)** — maintainer release, PyPI, tag, and marketplace checklist.
- **[SECURITY.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/SECURITY.md)** — vulnerability reporting.
- **[CODE_OF_CONDUCT.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/CODE_OF_CONDUCT.md)** — community guidelines.
...
- `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/hooks/unslop-mode-tracker.js` tracks a per-session turn counter (`~/.claude/.unslop-turn-count`) and re-emits an expanded reinforcement banner at turns 8, 16, 24, 32, and every 16 thereafter. No opt-in needed; the hook handles it. `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/hooks/unslop-activate.js` resets the counter on session start so nothing persists across shells.
- For voice-match, `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/scripts/style_memory.py` stores a numeric stylometric anchor on disk. Pure numbers, no free-text preferences. The MIT/Penn State CHI 2026 paper on "sycophancy memory" links free-text preference storage to amplified sycophancy over time. The cache makes that vector physically unavailable.
...
- [`docs/research/`](https://github.com/MohamedAbdallah-14/unslop/tree/v0.7.0/docs/research) — the public research index plus the August 2026 archive: 80 topic memos, 16 syntheses, four phase plans, and the maintainer handover.
- [`IMPLEMENTATION_TRACE.md`](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/docs/research/IMPLEMENTATION_TRACE.md) — every research finding mapped to the line of code it motivates.
...
flowchart LR
  subgraph SSOT ["Source of truth"]
    ...
    S2[https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/rules/unslop-activate.md]
    S3[https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/SKILL.md]
  end
...
**Source of truth:** `skills/unslop/SKILL.md`, the other authored `skills/*/SKILL.md` files, `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/rules/unslop-activate.md`, `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/SKILL.md`, `unslop/scripts/`, and root `CHANGELOG.md`. The `sync.yml` workflow propagates their generated counterparts on push to main.
...
python3 https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/tests/verify_repo.py              # Repo integrity (manifests, mirrors, syntax, fixtures)
python3 https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/run.py --strict        # Offline benchmark on AI-slop corpus, CI gates
```
...
- **`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/tests/test_hooks.py`** — hook installer (fresh, idempotent, preserves custom statusline), `unslop-activate.js` banner, `unslop-mode-tracker.js` slash commands + natural language + stop phrases, statusline badge output, symlink refusal, `CLAUDE_CONFIG_DIR` honoring.
- **`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/tests/verify_repo.py`** — every SSOT mirror is byte-identical after sync, JSON manifests parse, all JS / Bash / PowerShell scripts are syntax-clean, fixture pairs round-trip, plugin + marketplace manifests are wired.
- **`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/run.py`** — runs `humanize_deterministic` over a corpus of AI-slop markdown and reports AI-ism reduction, per-paragraph flat count, sentences split, bullet groups merged, per-file structural integrity. `--strict` fails the build on any regression.
- **`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/check_regression.py`** — compares latest benchmark output against a pinned `post-phase*.json` baseline. Fails if AI-ism reduction drops > 2 pp, flat-paragraph total rises > 2, or preservation breaks. Runs in CI on every PR.
- **`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/detector_bench.py`** — opt-in AI-detector benchmark (TMR, Desklib). Downloads HF weights on first run. Scheduled weekly via `.github/workflows/weekly-detector-bench.yml`.
- **`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/evals/perceived_humanness.py`** — blind LLM-as-judge preference harness. New runs default to Claude Sonnet 5; the published 21/21 result remains the archived Sonnet 4.5 run named above.
...
Living list. PRs welcome — see [CONTRIBUTING.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/CONTRIBUTING.md).
...
PRs welcome. Read [CONTRIBUTING.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/CONTRIBUTING.md) for the test gates and the SSOT sync rules — edit the source-of-truth files, not the mirrors, or CI will revert your change. The [CODE_OF_CONDUCT.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/CODE_OF_CONDUCT.md) applies.
...
Found a security issue? See [SECURITY.md](https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/SECURITY.md).
