...
The same skill ships across six interactive hosts: Claude Code, Cursor, Windsurf, Cline, Codex, and Gemini. GitHub Copilot receives the always-on rule as a passive repository instruction mirror. `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/scripts/sync-mirrors.sh` propagates all generated copies from the authored sources.
...
| Citation | arXiv | Used for |
|---|---|---|
| **A Survey on LLM-as-Judge** | [2411.15594](https://arxiv.org/abs/2411.15594) | `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/evals/perceived_humanness.py` A/B order randomization, multi-model jury (~40% position inconsistency, ~15% verbosity inflation) |
...
| Citation | arXiv | Used for |
|---|---|---|
| **HorizonBench** — Li et al. ("Long-Horizon Personalization with Evolving Preferences", 2026) | [2604.17283](https://arxiv.org/abs/2604.17283) | Drift-check banner cadence in `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/hooks/unslop-mode-tracker.js` (turns 8/16/24/32) |
...
- **`https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/tests/verify_repo.py`** — repo integrity verifier, runs as a CI step
...
| Codex plugin source bundle | `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/plugins/unslop/.codex-plugin/plugin.json` |
...
The authored skills, `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/SKILL.md`, `unslop/scripts/`, `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/rules/unslop-activate.md`, and root `CHANGELOG.md` are propagated to host-specific mirrors by `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/scripts/sync-mirrors.sh`, run by `.github/workflows/sync.yml` on push. Hand-editing a mirror is silently overwritten on next sync.

### Hooks

Four hook files in `hooks/`. CommonJS via `require()`. Communicate through a flag file at `$CLAUDE_CONFIG_DIR/.unslop-active` (falls back to `~/.claude/.unslop-active`). `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/hooks/package.json` pins `{"type":"commonjs"}` so hooks resolve `require()` even when an ancestor `package.json` declares `"type":"module"`.
...
- `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/run.py` + `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/check_regression.py` — quality + regression
- `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/detector_bench.py` — TMR + Desklib detector resistance
...
- `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/stylometric_baseline.py` — corpus-derived stylometric targets
- `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/evals/perceived_humanness.py` — LLM-judge eval, A/B randomized, multi-model jury
...
| Metric | Value | Source |
|---|---|---|
| AI-ism reduction (rule-counted) | **92.1%** | `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/benchmarks/results/latest.json` (9-fixture suite, 2026-04-28) |
| Core tests | 639 collected; paid API and real-model cases are opt-in | `pytest tests/unslop/ --collect-only -q` |
| Detector backbone (default) | TMR — 125M-param RoBERTa, **99.28% AUROC on RAID** | `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/scripts/detector.py:9` |
| Detector ladder | 4-step default (ends in `anti-detector`); 6-step aggressive variant | `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/scripts/detector.py` |
...
| File-size cap | 500 KB | `https://github.com/MohamedAbdallah-14/unslop/blob/v0.7.0/unslop/scripts/detect.py` |
