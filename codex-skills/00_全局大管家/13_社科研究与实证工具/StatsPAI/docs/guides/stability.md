...
```bash
python https://github.com/brycewang-stanford/StatsPAI/blob/v1.23.0/docs/guides/scripts/stability_audit.py
python https://github.com/brycewang-stanford/StatsPAI/blob/v1.23.0/docs/guides/scripts/stability_audit.py --unbacked
python https://github.com/brycewang-stanford/StatsPAI/blob/v1.23.0/docs/guides/scripts/stability_audit.py --check
python https://github.com/brycewang-stanford/StatsPAI/blob/v1.23.0/docs/guides/scripts/stability_audit.py --json
python Paper-JSS/replication/scripts/validation_evidence_audit.py
```

`https://github.com/brycewang-stanford/StatsPAI/blob/v1.23.0/docs/guides/scripts/stability_audit.py --check` fails if any hand-written stable API entry lacks attached validation or API/unit-contract evidence. Auto-registered entries are reported separately because they represent breadth imported into the registry, not the validated numerical core defended in the JSS paper.

The JSS packager also extracts Python source paths from registry evidence notes. The current submission manifest includes 320 such registry evidence files, and `Paper-JSS/replication/scripts/verify_submission_package.py` fails if any referenced evidence file is absent from the archive.
