...
- **计数护栏 lane**（2026-08-27）：`audit_repo.check_documented_counts` —— 把 capability
  ...
  文件放在同一个目录里。触碰面：`tools/audit_repo.py`、tools/tests 的 `test_documented_counts`（该测试目录已随集成瘦身移除）、
...
- **会议届次锚定修正 lane**（2026-08-27）：`cycle_audit.edition_years` 没有左边界，
  ...
  `tools/cycle_audit.py`、tools/tests 的 `test_cycle_audit`（该测试目录已随集成瘦身移除）、上述四个包的 source map、
...
- **外链体检 lane**（2026-08-26）：修 `external_link_audit.py` 把「站点对陌生
  ...
  tools/tests 的 `test_external_link_audit`（该测试目录已随集成瘦身移除）、`.maintenance/DEAD-LINKS.md`、
