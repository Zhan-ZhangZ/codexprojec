...
- **🔵 LOW** `LLM_HARMFUL_CONTENT` — Several referenced files are absent from the package
  > The instruction body and reference documents point to files that were not found in the package (e.g., assets/liquid-handling.md, assets/hardware-backends.md, templates/*.md, tests/pylabrobot/fixtures/protocol_manifest.json, https://github.com/K-Dense-AI/scientific-agent-skills/blob/v2.65.0/tests/pylabrobot/fixtures/transfers.csv). Missing referenced material is a documentation-integrity issue: an agent following the documented commands may fail, or could be tempted to fetch/synthesize substitutes. No malicious behavior is implied.
