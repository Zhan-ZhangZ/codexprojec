...
- When `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/check_docs_drift.py --strict` reports drift on `main` that
...
4. Apply each remediation as follows:
   ...
   | `gen-agents-md` | `uv run python https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/gen_agents_md.py --update`. Only run if the row explicitly names this token. Since #4019 the script refuses anyway when `agents-md-sync` owns AGENTS.md, so a misrouted run is a no-op rather than 74 deleted rows. |
...
5. After all rows in the partition are processed, run the drift gate:
   ...
   ```bash
   uv run python https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/check_docs_drift.py --strict
   ```
...
7. Push the branch and open a PR titled
   ...
   `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/check_docs_drift.py --strict` exits 0.
...
1. `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/check_docs_drift.py --strict` exits 0 on every open partition
...
2. For partitions that touched data-freshness lines (the `as of YYYY-MM-DD`
   ...
   `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/check_data_freshness.py` runs without hard failure (advisory
...
- `docs/compare/*` is operator-managed and out of scope for a sweep. The
  ...
  than a sweep pass.
- `CHANGELOG.md` and `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/CHANGELOG.md` are pointer documents naming
  ...
  `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/release-notes/unreleased.md` for what has landed since the newest
...
- Re-run `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/check_docs_drift.py --strict` locally on the latest
