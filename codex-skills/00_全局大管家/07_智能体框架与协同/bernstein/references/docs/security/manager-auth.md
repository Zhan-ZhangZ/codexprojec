...
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/security/auth_middleware.py`).
...
| File | What it does |
|------|--------------|
| `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/agents/spawner_core.py` (`_issue_agent_token`, `_render_auth_section`) | Mints the per-session JWT, writes it `0600`, injects path into the manager prompt. |
| `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/adapters/env_isolation.py` (`_BASE_ALLOWLIST`) | Allowlist for cross-process env inheritance. Contains `BERNSTEIN_AUTH_TOKEN`, `BERNSTEIN_HOOK_SECRET`, `BERNSTEIN_AUTH_DISABLED`. |
| `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/security/auth.py` (`AuthService`, `create_jwt`, `verify_jwt`) | JWT mint/verify (HS256), legacy bearer constant-time compare. |
| `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/security/auth_middleware.py` (`SSOAuthMiddleware`, `auth_disabled_via_opt_out`) | Server-side gate. Implements the three-strategy chain (SSO JWT → agent JWT → legacy bearer). |
| `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/server/server_app.py` (`create_app`) | Resolves `BERNSTEIN_AUTH_TOKEN` at boot, wires `legacy_auth_token` and `AgentIdentityStore` into middleware. |
| `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/routes/auth.py` | `/auth/providers`, `/auth/cli/device`, `/auth/cli/token` (device-flow for human CLI logins - not used by spawned agents). |
...
- **#1261** - *Manager agent cannot authenticate to task server to spawn
  ...
  `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/orchestration/manager.py` does not yet attach a
