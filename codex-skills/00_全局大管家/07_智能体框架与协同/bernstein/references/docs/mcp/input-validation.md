...
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/mcp/server.py` replaces each tool's `inputSchema` with the
...
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/unit/mcp/test_advertised_schema_parity.py` is the drift guard. It
...
`_apply_tool_timeouts` in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/mcp/server.py`) and, when the
...
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/mcp/input_validation.py`), the `role` property's `enum` is
...
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/mcp/input_validation.py` (validator, deny rules, schema
...
schemas); wired into request handling in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/mcp/server.py`
(`_validate_or_error`) and advertised from the same files by
`_apply_advertised_schemas`. `bernstein_verify_lineage` validates in
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/mcp/resources/lineage.py`.
