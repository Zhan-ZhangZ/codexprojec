...
The mock adapter (`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/adapters/mock.py`) supports an **idle mode** that sleeps each spawned agent for a randomized interval instead of calling any LLM. Combined with a sibling fixture repo, this gives you a continuous stream of mock tasks and agents that keep the GUI populated.
...
- Mutually exclusive with `--dry-run`. Defined in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/cli/run_bootstrap.py` (`--idle` option, lines ~842–1030).
