...
Auth source: `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/security/auth_middleware.py`. Idle source: `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/cli/run_bootstrap.py` and `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/adapters/mock.py`.
...
- Tokens are defined in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/web/src/index.css`.
- The theme respects `prefers-color-scheme` on first load and persists the operator's explicit choice via `localStorage`.
- `prefers-reduced-motion: reduce` disables non-essential transitions (motion spec in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/web/src/lib/motion.ts`).

## Fleet mode toggle

- The topbar contains a `Single / Fleet` segmented control (defined in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/web/src/components/AppShell.tsx`).
...
- The toggle is a UI affordance only; it does not change auth or routing on its own. Configure the fleet endpoint upstream - see [Fleet (multi-project)](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/fleet.md).

## Build chip and `/health/deps`

The footer build chip reads `GET /api/v1/gui-meta` for `version` / `commit` / `build_time`, and `GET /health/deps` for the green/red status dot. Source: `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/gui/__init__.py` (`gui_meta` route).
