...
The CLI is defined in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/gui/cli.py`. The mount logic is `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/gui/__init__.py` (`mount(app)` attaches the SPA at `/ui/` and registers `/api/v1/gui-meta`).
