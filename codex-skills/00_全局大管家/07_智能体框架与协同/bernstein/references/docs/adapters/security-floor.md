...
- **Enforced at spawn.** Before a tracked adapter is spawned, its installed
  ...
  `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/adapters/advisories.py`. A below-floor binary is refused by
...
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/adapters/advisories.py` (regenerated between the floor-map
markers), accompanied by a signed update receipt that binds the old and new
floor-map content hashes:

```
uv run python https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/scripts/refresh_adapter_floors.py --feed feed.json \
