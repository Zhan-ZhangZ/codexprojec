...
Source of truth: `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/tasks/lifecycle.py` (transition tables),
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/tasks/models.py` (`TaskStatus` enum, `AgentSession` dataclass).
...
> **Note - `PENDING_APPROVAL`:** This state exists in the `TaskStatus` enum and is used by the approval subsystem (see `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/security/approval.py`). It is set directly rather than through the `TASK_TRANSITIONS` table, so it has no FSM-managed entry or exit path. Tasks in this state await human review and cannot progress further without manual intervention.
...
Task timeouts are not static. The adaptive timeout system (`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/orchestration/adaptive_timeout.py`) adjusts wall-clock timeouts based on historical task durations. Default scope-based timeouts are defined in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/defaults.py` (`TASK.scope_timeout_s`): small=15 min, medium=30 min, large=60 min, XL=120 min.

### Agent Trust Tiers

Per-agent trust tiers (`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/agents/agent_trust.py`) track each role's completed tasks, failures, and security violations in `.sdd/trust/<agent_id>.json`, and map each tier onto an `AgentPermissions` profile. Trust is recorded when a task completes or fails and is inspectable with `bernstein agents trust`.
...
Source: `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/tui/agent_states.py` (`AgentState`, `classify_agent_state`).
...
Source of truth: `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/src/bernstein/core/agents/agent_turn_state.py` (`AgentTurnState`,
