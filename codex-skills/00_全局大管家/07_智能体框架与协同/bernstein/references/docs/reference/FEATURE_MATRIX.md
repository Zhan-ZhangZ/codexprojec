...
surface an operator can invoke. `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/unit/test_cli_server_route_parity.py`
...
`https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/unit/test_feature_matrix_drift.py` holds the CLI-commands section to
...
| [Typed activity boundary](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/activity-boundary.md) | Full | 3 | One hash-in/hash-out contract for coding, research, browser, data, and ops activities, verified by `bernstein activity verify <run>` (`core/orchestration/activity.py`). Browser is the one non-coding modality with a CLI dispatch verb (`bernstein activity browser run`); the rest are Python-API only. Dispatching a non-coding modality from a seed, plan, or backlog file is not wired yet; see [Reachability today](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/activity-boundary.md#reachability-today) |
...
| [Tournament runs](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/tournament-runs.md) | Full | 3 | Parallel attempts selected by deterministic evaluators; the winner carries a signed selection receipt (`core/tournament/`) |
| [Fleet steering](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/fleet-steering.md) | Full | 3 | Pause, resume, guidance, redirect, and abort each land a signed steering receipt before any effect runs (`core/orchestration/steering.py`) |
| [Detached run service](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/run-service.md) | Full | 3 | Submit a goal, disconnect, and reattach later against a supervised run service (`core/run_service/`) |
| [Named resource pools](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/named-resource-pools.md) | Full | 3 | Lease-backed admission with chain-anchored grant and release receipts (`core/admission/`) |
...
| [Session persistence (fast resume)](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/session-fast-resume.md) | Full | 3 | `session.py` resumes after stop/restart |
...
| [Prometheus `/metrics`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/observability-overview.md) | Full | 3 | Endpoint is real; Grafana dashboards are user-defined |
| [OTLP telemetry initialization](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/observability-overview.md) | Full | 3 | Wiring exists in `core/observability/` |
| [OTel GenAI span projection](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/observability-overview.md) | Full | 3 | `trace project` projects a run journal into signed OTel GenAI spans; `trace verify-projection` recomputes span ids |
...
| [Session analytics](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/session-analytics.md) | Full | 3 | `bernstein recap` shows session-level stats |
| [Debug bundle](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/debug-bundle.md) | Full | 4 | `bernstein debug` collects logs/state/config for triage |
...
| [Token growth monitor](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/token-growth-monitor.md) | Full | 3 | Auto-intervention on runaway consumption |
| [Cost anomaly detection](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/cost-anomaly-detection.md) | Full | 3 | Z-score based, acts via task completion |
| [Agent loop detection](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/agent-loop-detection.md) | Full | 3 | Kills agents in edit-loop cycles |
...
| [Behaviour anomaly detection](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/observability-overview.md) | Full | 3 | Flags agents whose runtime metrics deviate statistically from baseline (`core/observability/behavior_anomaly.py`) |
...
| [Gate adjudication records](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/gate-adjudication.md) | Full | 3 | Gate panel decisions are recomputable; `gate verify` confirms the inputs hash |
| [In-process verification gates](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/hook-gate.md) | Full | 3 | Verification-gate hooks rendered into capable adapters so the check runs inside the agent (`adapters/hook_gate_render.py`) |
...
| [Review / autofix / escalation / consent / webhook-node receipts](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/review-receipts.md) | Full | 3 | Signed, journal-anchored receipts verified offline (`review-receipt verify`, `escalation verify`, `webhook verify`) |
...
| [Stall escalation receipts](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/stall-escalation.md) | Full | 3 | A stalled worker produces a signed escalation receipt embedding the last audit entries and a deterministic recommended action (`supervisor escalate`) |
...
| [Agent run manifest](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/run-manifest.md) | Full | 3 | Hashable workflow spec for SOC 2 evidence |
| [RBAC / budget / seat projections](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/governance.md) | Full | 3 | Access and budget verdicts re-derivable via `governance verify` |
| [Unified event feed](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/events/grammar.md) | Full | 3 | Chain-projected event feed with a typed grammar and per-event receipts (`core/events/feed.py`) |
...
| [Attenuated capability tokens](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/security-and-identity.md) | Full | 4 | Each delegation hop is a signed, scope-attenuating token, so the principal to orchestrator to sub-agent authority chain verifies offline (`core/security/capability_tokens.py`) |
...
| [Authorized-action mandates](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/spending-mandates.md) | Full | 3 | `mandate emit/verify/revoke` binds, proves, and revokes an authorized-action mandate |
...
| [Budget envelopes](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/cost-envelopes.md) | Full | 4 | Per-phase and per-task budget envelopes with rollup by envelope (`core/cost/cost_rollup_by_envelope.py`) |
...
| [Trigger sources](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/trigger-sources.md) | Full | 3 | `github`, `gitlab`, `slack`, `discord`, `file_watch`, `webhook`, `odata`, and `schedule` source adapters (`core/trigger_sources/`) |
| [OData trigger source](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/odata.md) | Full | 3 | Polls an OData endpoint into normalized trigger events (`core/trigger_sources/odata_poll.py`) |
| [Webhook node](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/webhook-node.md) | Full | 3 | Outbound webhook node with recomputable inbound-event and node hashes (`core/trigger_sources/webhook_node.py`, `webhook verify`) |
...
| Cluster/worker primitives | Full | 3 | `bernstein worker --server URL`, cluster routes documented. A worker registering against a live cluster-enabled server is covered by `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/integration/test_first_run_long_running_surfaces.py`, which is the same code path the `bernstein worker` CLI row rests on. |
...
| [Credential vault](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/secrets.md) | Full | 3 | `bernstein connect <provider>`, `bernstein creds list/revoke/test` OS-keychain token storage (`core/security/vault/`) |
| [Autofix CI daemon](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/autofix.md) | Full | 3 | `bernstein autofix start/stop/status/attach` watches PRs and dispatches repair runs on CI failure (`core/autofix/`) |
| [Dev preview](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/preview.md) | Full | 3 | `bernstein preview start/stop/list/status` exposes an agent dev server via tunnel with configurable auth (`core/preview/`) |
| [Fleet dashboard](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/fleet.md) | Full | 3 | `bernstein fleet [--web HOST:PORT]` cross-session multi-instance view (`core/fleet/`) |
| [Notification sinks](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/notifications.md) | Full | 3 | `bernstein notify test --sink <id>` pluggable notification backends (`core/notifications/`) |
| [PR review responder](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/review-responder.md) | Full | 3 | `bernstein review-responder start/status/tick` auto-responds to PR review comments (`core/review_responder/`) |
| [Review pipeline DSL](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/review-pipeline.md) | Full | 3 | `bernstein review --pipeline review.yaml` YAML-driven multi-phase review, plus the bounded `--fix --until-checks-green` contour and its per-pass receipts (`core/quality/review_pipeline/`) |
| [Plan archival](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/plan-archival.md) | Full | 3 | `bernstein plan ls/show` list and inspect archived plans (`core/planning/lifecycle.py`) |
| [Slack integration](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/slack-webhooks.md) | Full | 3 | Slash commands and events API endpoints |
...
| [Workflow DSL](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/workflow-manifests.md) | Full | 3 | `bernstein workflow validate/list/show` |
| [Chaos engineering](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/chaos-engineering.md) | Full | 3 | `bernstein chaos agent-kill/file-remove/status/slo` |
| Benchmark suite | Full | 4 | `bernstein eval run/compare/swe-bench/programbench` |
| [Eval harness](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/eval/golden-harness.md) | Full | 4 | `bernstein eval run/report/failures` |
| SWE-Bench harness | Full | 4 | Verified eval in `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/benchmarks/swe_bench/run.py` |
| [Graduation system](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/graduation.md) | Full | 3 | Agent promotion stages, routes in `routes/graduation.py`. The documented REST surface is driven end to end against a real server by `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/integration/test_first_run_graduation_surface.py`: policies, record-event, eligibility at the sandbox threshold, and promotion to `shadow`. |
...
| [Prompt caching](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/performance-tuning.md) | Full | 3 | SHA-256 system prefix deduplication |
...
| [Installation mismatch detection](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/doctor.md) | Full | 4 | Detects adapter/installation gaps |
| [Worker badge identity](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/worker-process-identity.md) | Full | 3 | Process identification in `ps`/Activity Monitor |
| [Keybinding system (TUI)](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/tui-keybindings.md) | Full | 3 | Configurable TUI keyboard shortcuts. `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/unit/test_first_run_keybinding_overrides.py` resolves the documented three layers and then presses the overridden key against a running Textual app, asserting the bound action fires and the pre-override key no longer does. |
...
| [Commit attribution stats](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/commit-attribution.md) | Full | 3 | Per-agent commit statistics, via `bernstein report commits` |
...
| `bernstein init` | Full | 3 | Workspace setup. The documented first run is covered by `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/integration/test_first_run_documented_path.py`, which runs the command from an empty directory and asserts the created artifacts plus the key output lines documented in `first-run.md`. |
| `bernstein stop` | Full | 3 | Graceful/force stop |
| `bernstein live` | Full | 3 | TUI dashboard. Readiness is the first rendered frame, identified by the `AGENTS` and `TASKS` pane headers; `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/integration/test_first_run_long_running_surfaces.py` starts it from an empty workspace, waits for that frame, and asserts a traceback-free exit on `SIGINT`. |
...
| `bernstein report commits/incident/postmortem` | Brief | 3 | Per-run markdown summaries: `commits` is per-agent commit attribution ([reference](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/commit-attribution.md)); `incident` correlates a timeline from logs, metrics, and traces; `postmortem` writes a structured report for a failed run. The group has no reference page of its own; `cli-reference.md` and `bernstein report --help` carry it |
...
| [`bernstein replay ID`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/replay.md) | Full | 3 | Deterministic replay |
| [`bernstein checkpoint`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/checkpoint.md) | Full | 3 | Session snapshot |
| [`bernstein wrap-up`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/wrap-up.md) | Full | 3 | End session with summary |
...
| [`bernstein worker`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/cluster-mode.md) | Full | 3 | Join cluster as worker. Readiness is the `Registered as node <id>` line; `https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/tests/integration/test_first_run_long_running_surfaces.py` registers one against a live cluster-enabled server and separately asserts the non-git-workspace refusal names its reason. |
| [`bernstein mcp`](../mcp/server.md) | Full | 3 | Run as MCP server |
| [`bernstein chaos`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/chaos-engineering.md) | Full | 3 | Fault injection |
...
| [`bernstein eval`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/eval/golden-harness.md) | Full | 4 | Evaluation harness |
| `bernstein workspace` | Full | 3 | Multi-repo workspace |
| [`bernstein config`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/global-config.md) | Full | 3 | Configuration management |
...
| [`bernstein sync`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/backlog-sync.md) | Full | 3 | Sync backlog with server |
| [`bernstein manifest`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/run-manifest.md) | Full | 3 | Run manifest inspection |
| [`bernstein gateway`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/mcp-gateway.md) | Full | 3 | MCP gateway proxy |
| [`bernstein workflow`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/workflow-manifests.md) | Full | 3 | Workflow DSL |
| [`bernstein watch`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/watch.md) | Full | 3 | Directory file watcher |
| [`bernstein listen`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/voice-control.md) | Full | 2 | **Preview.** Voice commands are experimental. A base install exits with the `pip install 'bernstein[voice]'` hint; a usable first run also requires microphone/audio support and downloads the selected Whisper model on first use. |
| [`bernstein completions`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/shell-completions.md) | Full | 3 | Shell completion scripts |
| [`bernstein self`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/updates.md) | Full | 3 | Provenance-verified update lifecycle: signed feed, chain-anchored advisory, pre-install wheel verification, pin, receipted rollback |
| [`bernstein self-update`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/self-update.md) | Full | 3 | Compatibility alias for `bernstein self` |
| [`bernstein plugins`](../integrations/plugin-sdk.md) | Full | 3 | List active plugins |
| [`bernstein install-hooks`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/contributing/git-hooks.md) | Full | 3 | Install git hooks |
| [`bernstein debug`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/debug-bundle.md) | Full | 4 | Generate debug bundle for triage |
| `bernstein acp serve` | Full | 3 | ACP bridge (`--stdio` or `--http :PORT`) |
| [`bernstein autofix ...`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/autofix.md) | Full | 3 | CI autofix daemon (start/stop/status/attach) |
| [`bernstein connect`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/secrets.md) | Full | 3 | Credential vault setup for a provider |
| [`bernstein creds ...`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/secrets.md) | Full | 3 | Credential management (list/revoke/test) |
| [`bernstein preview ...`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/preview.md) | Full | 3 | Dev server preview (start/stop/list/status) |
| [`bernstein fleet`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/fleet.md) | Full | 3 | Fleet dashboard (optionally `--web HOST:PORT`) |
| [`bernstein fleet steer`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/fleet-steering.md) | Full | 3 | Mid-run steering: pause/resume/guidance/redirect/abort |
| [`bernstein mcp catalog ...`](mcp-catalog.md) | Full | 3 | MCP catalog browser (browse/search/install) |
| [`bernstein notify test`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/notifications.md) | Full | 3 | Notification sink smoke test |
| [`bernstein plan ls/show`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/plan-archival.md) | Full | 3 | List and inspect archived plans |
| [`bernstein review-responder ...`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/review-responder.md) | Full | 3 | PR review responder (start/status/tick) |
| [`bernstein review --pipeline`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/review-pipeline.md) | Full | 3 | Review with YAML pipeline DSL |
| [`bernstein fork --run --from-step`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/fork-from-step.md) | Full | 3 | Fork a run at a journal step into a new isolated run |
| [`bernstein gate verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/gate-adjudication.md) | Full | 3 | Recompute a gate panel's inputs hash and confirm the adjudication |
| [`bernstein mandate emit/verify/revoke`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/spending-mandates.md) | Full | 3 | Bind, prove, and revoke authorized-action mandates |
| `bernstein payment-mandate issue/show/spend` | Full | 3 | Issue and spend against authorized-spend mandates with signed receipts |
| [`bernstein governance verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/governance.md) | Full | 3 | Recompute access and budget verdicts for a run |
| [`bernstein webhook verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/webhook-node.md) | Full | 3 | Recompute inbound-event and outbound webhook-node hashes |
| [`bernstein review-receipt emit/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/review-receipts.md) | Full | 3 | Bind and offline-verify PR review receipts (issue + plan + tool calls + diff) |
...
| [`bernstein escalation show/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/stall-escalation.md) | Full | 3 | Project and reconstruct escalation receipts from the journal |
| [`bernstein supervisor status/escalate`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/api/supervisor.md) | Full | 3 | Supervise stalled workers and seal stall escalation receipts |
| [`bernstein delegation verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/delegation-verify.md) | Full | 4 | Reconstruct and verify a run's delegation chain |
| [`bernstein credential emit/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/content-credentials.md) | Full | 3 | Project an artifact's lineage into a signed C2PA credential and verify it |
| [`bernstein skills provenance/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/skill-provenance.md) | Full | 3 | Recompute a skill's install receipt and usage-provenance graph |
| [`bernstein schedule verify/audit`, `schedule show --at`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/schedule.md) | Full | 3 | Replay recorded fires, chain-check fire receipts, project a schedule at a time |
...
| [`bernstein thread verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/deterministic-replay.md) | Full | 3 | Prove a streamed TUI thread equals its executed journal |
| `bernstein memory verify/why/forget` | Full | 4 | Prove authorship, trace origin, and tombstone a memory entry |
| [`bernstein replay --verify/--from-step`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/deterministic-replay.md) | Full | 3 | Recompute the journal head or rebuild state to a step |
| [`bernstein lineage verify/walk/chain/replay/export`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/lineage.md) | Full | 4 | Per-artifact lineage spine: recompute a run's Merkle chain and HMAC tags, walk an artifact back to its producer, replay the spine in append order, and export a run's chain as a regulator artefact (`tracker-audit`, `forks`, `conflicts`, `resolve`, `merge`, `reindex`, `gate`, and `v2` complete the group) |
...
| [`bernstein evidence show/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/evidence-bundles.md) | Full | 3 | Project and verify a sealed evidence bundle |
| [`bernstein events query/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/events/grammar.md) | Full | 3 | Query the unified event feed and verify its chain projection |
...
| [`bernstein tournament show/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/tournament-runs.md) | Full | 3 | Inspect a tournament run and verify its selection receipt |
...
| [`bernstein run-service submit/attach/status`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/run-service.md) | Full | 3 | Submit a detached run, then reattach to it later |
...
| [`bernstein pool register/list/show/verify`](https://github.com/sipyourdrink-ltd/bernstein/blob/v3.19.0/docs/operations/sandbox-pools.md) | Full | 3 | Manage lease-backed named resource pools |
