---
name: openmaic
description: "OpenMAIC 清华大学开源多智能体交互式数字课堂与课件生成全栈套件。支持通过单一主题或上传文档/音视频一键生成包含 AI 教师与 AI 同学的互动教学课堂（含幻灯片演讲、白板演算推演、实时问答互动、测验 Quiz、交互式仿真与 PBL 项目制学习）。提供官方 Web 端与 Pro 智能体工作台、云端 Live Demo 快速接入、本地与 Docker 容器化部署、全模态模型与 TTS 语音合成调度，并支持通过 @openmaic/* SDK 进行二次开发与课件 DSL 渲染定制。Leading Words: OpenMAIC, 多智能体交互课堂, 智能体课件生成, AI虚拟教师, 交互式仿真学习, 白板推演与语音讲解, PBL项目制学习, 课件DSL渲染"
user-invocable: true
