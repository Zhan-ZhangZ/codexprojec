...
A local [MCP](https://modelcontextprotocol.io) server exposing 299 tools across 100% of eBay's Sell APIs. TypeScript/Node.js (ESM), built with `@modelcontextprotocol/sdk`, Effect-backed validation, a Zod SDK adapter, and OpenAPI-generated types.

- **Entry points:** [`index.ts`](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/src/index.ts) (STDIO transport — default) and [`serverHttp.ts`](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/src/serverHttp.ts) (HTTP transport).
...
- **GitHub Copilot:** [https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/.github/copilot-instructions.md](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/.github/copilot-instructions.md) keeps Copilot-specific reminders and points back here.
...
| `auth/`          | OAuth 2.0 flow, token management, and setup callback helper (`oauthHelper`)                                                                                                                                                                                                                                                                  |
...
| `scripts/`       | CLI tooling: setup/diagnostics, wizards/validators, `devSync`, `skills`                                                                                                                                                                                                                                                                      |
| `utils/`         | Platform primitives (logging, http, errors, effectSchema*, cliUi) — not setup UX                                                                                                                                                                                                                                                             |
...
- **Tool exposure** is gated by `EBAY_MCP_TOOLS` (`all` | `dynamic` | family list). The env parsing/validation lives in [`toolFamilies.ts`](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/src/config/toolFamilies.ts) (kept free of tool-tree imports to avoid a cycle with `config/environment.ts`); the dynamic-mode discovery meta-tools and catalogue live in [`toolGating.ts`](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/src/mcp/toolGating.ts); [`runtime.ts`](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/src/mcp/runtime.ts) applies the mode. Family keys must stay in sync with `toolCategories` (a unit test enforces this).
