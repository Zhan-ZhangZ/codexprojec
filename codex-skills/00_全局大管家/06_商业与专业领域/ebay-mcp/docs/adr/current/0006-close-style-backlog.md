...
- **`ui/`.** Fix the 29 `class`→`className` type errors (React 19 tolerates
  ...
  [`tsconfig.json`](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/ui/tsconfig.json) (was untracked); add `typecheck:ui` to the CI **Typecheck**
...
- **Docs.** Acknowledge `HttpError` ([`http.ts`](https://github.com/YosefHayim/ebay-mcp/blob/v1.15.0/src/utils/http.ts)) as the one sanctioned
