...
function Invoke-Staging([string]$SourcePath, [string]$InstructionsPath) {
    $stagingScript = Join-Path $PSScriptRoot "stage_review_inputs.ps1"
...
function FindTargets($Paragraphs, $Operations) {
    ...
    foreach ($op in $Operations) {
        ...
        if (-not $used.Add($idx)) { throw "同一段落被重复命中: $($op.AnchorText)" }
...
function ValidateDocx([string]$Docx, [bool]$NeedComments, [bool]$NeedRevisions) {
    ...
    try {
        ...
        if ($NeedRevisions) {
            ...
            if ($null -eq (SelectOneNs -XmlObject $doc -XPath "//w:ins") -or $null -eq (SelectOneNs -XmlObject $doc -XPath "//w:del")) { throw "输出文件未写入修订痕迹: $Docx" }
        }
...
try {
    ...
    foreach ($raw in @($config.operations)) {
        ...
        if ([string]::IsNullOrWhiteSpace([string]$raw.anchor_text)) { throw "operation 缺少 anchor_text" }
        ...
        $operations += [pscustomobject]@{ AnchorText = [string]$raw.anchor_text; Mode = $mode; ReplacementText = $(if ($null -ne $raw.PSObject.Properties["replacement_text"]) { [string]$raw.replacement_text } else { $null }); Comment = $comment; CommentLines = $commentLines; MatchType = $matchType; Occurrence = $occurrence }
    }
