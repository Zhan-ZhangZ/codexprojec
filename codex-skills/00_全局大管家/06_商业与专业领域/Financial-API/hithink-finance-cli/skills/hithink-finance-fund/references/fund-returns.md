# `hithink-finance fund returns`

## 前置条件

- 先读取本 skill 的 `SKILL.md` 和 `../hithink-finance-shared/SKILL.md`。
- 执行前用 `hithink-finance schema fund.returns --format json` 确认当前参数契约。
- 远端命令需要 API Key；认证失败时回到 shared skill。

## 命令

```bash
hithink-finance schema fund.returns --format json
hithink-finance fund returns --fund-type <type> --thscode <code> --format json
```
