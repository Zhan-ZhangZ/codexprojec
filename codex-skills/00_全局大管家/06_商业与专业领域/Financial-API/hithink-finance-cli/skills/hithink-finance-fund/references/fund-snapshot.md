# `hithink-finance fund snapshot`

## 前置条件

- 先读取本 skill 的 `SKILL.md` 和 `../hithink-finance-shared/SKILL.md`。
- 执行前用 `hithink-finance schema fund.snapshot --format json` 确认当前参数契约。
- 远端命令需要 API Key；认证失败时回到 shared skill。

## 命令

```bash
hithink-finance schema fund.snapshot --format json
hithink-finance fund snapshot --thscode <code> --format json
```
...
| 参数               | 必填 | 说明                                            |
| ------------------ | ---- | ----------------------------------------------- |
| `--thscode <code>` | 是   | single ETF or LOF thscode                       |
