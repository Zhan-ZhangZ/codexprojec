---
source: "Espressif ESP32-S3 HW Design Guidelines -- PCB Layout (SPI/Flash Section)"
...
This chapter introduces the key points of how to design an ESP32-S3 PCB layout using an ESP32-S3 module (see Figure [ESP32-S3 Reference PCB Layout](#fig-pcb-layout-example)) as an example.
...
Figure [ESP32-S3 Power Traces in a Four-Layer PCB Design](#fig-power-layout-four-layer) shows the power traces in a four-layer PCB design.
...
* The 3.3 V power traces, highlighted in yellow, are routed as shown in Figure [ESP32-S3 Power Traces in a Four-Layer PCB Design](#fig-power-layout-four-layer). The width of the main power traces should be no less than 25 mil. The width of VDD3P3 at pin2 and pin3 power traces should be no less than 20 mil. The recommended width of other power traces is 10 mil. Ensure the power traces are surrounded by ground copper.
* The red circles in [ESP32-S3 Power Traces in a Four-Layer PCB Design](#fig-power-layout-four-layer) show ESD protection diodes. Place them close to the power input. Add a 10 µF capacitor before the power trace enters the chip. You can also add a 0.1 µF or 1 µF capacitor in parallel. After that, the power trace can branch out in a star-shaped layout to reduce coupling between different power pins.
...
ESP32-S3 EPAD Design at Chip Bottom
...
Figure [ESP32-S3 Crystal Layout (with Keep-out Area on Top Layer)](#fig-crystal-not-to-gnd) shows a reference PCB layout where the crystal is connected to the ground through vias and a keep-out area is maintained around the crystal on the top layer for ground isolation.

ESP32-S3 Crystal Layout (with Keep-out Area on Top Layer)

Figure [ESP32-S3 Crystal Layout (without Keep-out Area on Top Layer)](#fig-crystal-to-gnd) shows the layout for the crystal that is connected to the ground through vias but there is no keep-out area on the top layer for ground isolation.
...
The RF trace is routed as shown highlighted in pink in Figure [ESP32-S3 RF Layout in a Four-layer PCB Design](#fig-rf-layout).
...
* It is recommended to keep all layers clear under the IPEX antenna connector. See Figure [ESP32-S3 IPEX Layout](#fig-pcb-ipex-layout).
...
* If the flash and PSRAM are located far from ESP32-S3, it is recommended to place appropriate decoupling capacitors both at VDD\_SPI and near the flash and PSRAM power supply.

Figure [ESP32-S3 Quad SPI Flash Layout](#fig-flash-layout) shows the quad SPI flash layout.

ESP32-S3 Quad SPI Flash Layout

Figure [ESP32-S3 Octal SPI Flash Layout](#fig-octal-flash-layout) shows the octal SPI flash layout.

ESP32-S3 Octal SPI Flash Layout

## UART

Figure [ESP32-S3 UART Layout](#fig-uart-layout) shows the UART layout.
...
Figure [ESP32-S3 Typical Touch Sensor Application](#fig-touch-design) depicts a typical touch sensor application.
...
Figure [ESP32-S3 Electrode Pattern Requirements](#fig-touch-electrode) shows the proper and improper size or shape of electrode. Please note that the examples illustrated in the figure are not of actual scale. It is suggested to use a human fingertip as reference.

ESP32-S3 Electrode Pattern Requirements

### PCB Layout

Figure [ESP32-S3 Sensor Track Routing Requirements](#fig-touch-sensor-routing) illustrates the general guidelines to routing traces. Specifically,
...
ESP32-S3 touch sensor has a waterproof design and features proximity sensor function. Figure [ESP32-S3 Waterproof and Proximity Sensing Design](#fig-touch-waterproof-design) shows an example layout of a waterproof and proximity sensing design.
