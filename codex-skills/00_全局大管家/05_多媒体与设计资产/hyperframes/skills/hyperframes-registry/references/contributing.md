...
- **HeyGen internal contributors:** run [upload-docs-images.sh](https://github.com/heygen-com/hyperframes/blob/v0.8.25/scripts/upload-docs-images.sh) (requires AWS profile `engineering-767398024897`)
...
```bash
# 0. The generator scripts live in the upstream repo — work in a checkout of
#    https://github.com/heygen-com/hyperframes (tag v0.8.25 or newer):
...
# 3. Regenerate registry.json (see
#    https://github.com/heygen-com/hyperframes/blob/v0.8.25/registry/registry.json)
#    from the item directories.
...
npx tsx "$UPSTREAM/scripts/generate-registry-items.ts"

# 4. Generate catalog docs page
npx tsx "$UPSTREAM/scripts/generate-catalog-pages.ts"

# 5. Publish to hyperframes.dev so reviewers can preview
npx hyperframes publish

# 6. Stage everything (including the regenerated
#    https://github.com/heygen-com/hyperframes/blob/v0.8.25/registry/registry.json)
git add registry/{kind}/{name}/ docs/catalog/
...
- [ ] [`registry.json`](https://github.com/heygen-com/hyperframes/blob/v0.8.25/registry/registry.json) updated with new entry
- [ ] [`generate-catalog-pages.ts`](https://github.com/heygen-com/hyperframes/blob/v0.8.25/scripts/generate-catalog-pages.ts) run (docs page generated)
