...
MCP client (Claude Desktop, Codex, Cursor, OpenCode)
    │  JSON-RPC over stdio
    ▼
https://github.com/kridaydave/File-Organizer-MCP/blob/v5.0.0/src/server.ts            createServer() → fresh McpServer, registerTool loop
    │
https://github.com/kridaydave/File-Organizer-MCP/blob/v5.0.0/src/mcp/registry.ts      TOOLS + handler map (explicit list, no auto-discovery)
    │
https://github.com/kridaydave/File-Organizer-MCP/blob/v5.0.0/src/mcp/context.ts       ctx = { config, history } built fresh per request
    │
...
├── extensions/scheduler/  cron watch daemon + its own bin (https://github.com/kridaydave/File-Organizer-MCP/blob/v5.0.0/bin/file-organizer-watch.mjs)
...
Implementation: `https://github.com/kridaydave/File-Organizer-MCP/blob/v5.0.0/src/services/path-validator.service.ts` (`validateStrictPath`). Allowed roots are platform-aware and user-configured; nothing hardcodes a home directory.
...
Errors crossing the wire pass through `sanitizeErrorMessage()` (`https://github.com/kridaydave/File-Organizer-MCP/blob/v5.0.0/src/utils/error-handler.ts`). Internal paths never appear in tool responses; handlers throw `ValidationError` / `AccessDeniedError` and `createErrorResponse` formats them.

## Adding a tool

One new file plus one registry line. Create `src/tools/my-tool.ts` exporting a `ToolDefinition` + handler, then add an import and one `reg()` entry in `https://github.com/kridaydave/File-Organizer-MCP/blob/v5.0.0/src/mcp/registry.ts`. Schemas go in `src/schemas/`. Nothing else changes: the registry header documents this contract, and auto-discovery was deliberately rejected (fs-scanning `dist/` trades a visible one-line edit for invisible wiring).
